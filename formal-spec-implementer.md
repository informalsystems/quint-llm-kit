...
2. **Study the Protocol Description** (if provided): The user may provide a protocol/algorithm description document that:
   ...
   **IMPORTANT**: Pay extremely close attention to any mappings between abstract specification concepts and concrete implementation details. For example:
   - "The spec uses Set[Message] but implementation should use a HashMap keyed by sender for O(1) lookup"
   - "The spec checks all N nodes but implementation can short-circuit after finding quorum"
   - "The spec's 'justification' field should be implemented as a BTreeSet to maintain deterministic ordering"
   - "Byzantine behavior in spec is for model checking; don't implement the `update_byzantine` function"

   These implementation notes are authoritative and must be followed - incorporate them into every relevant task.
...
5. **Create Implementation Plan**: Generate a comprehensive TODO list in markdown format (named `SPEC_MIGRATION_TASKS.md`) organized by the `main_listener` function from the target spec.
   ...
   **Implementation Guidance** (from protocol description):
   - [Any specific mappings from abstract spec to concrete code]
   - [Data structure choices, optimizations, or clarifications]
   - [Things to avoid or special considerations]
