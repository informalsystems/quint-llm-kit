...
### Optional Parameters
- `max_implementation_attempts`: Max validation retry attempts (default: 3)
- `max_verification_iterations`: Max verification retry iterations (default: 2)
...
```json
{
 ...
 "attempts_exhausted": {
 "implementer_validation": 3,
 "verifier_iteration": 2
 },
 "partial_results": {
 "analyzer": "completed",
...
6. **Initialize Attempt Counter**
 ...
 - Set: max_attempts = max_implementation_attempts

7. **Per Attempt** (while implementation_attempt < max_attempts):
 ...
 g. **Check Loop Condition**
 - If implementation_attempt >= max_attempts:
 - Break loop, proceed to step 8

8. **Implementation Failure Escalation** (if max_attempts reached)
 - Record: All validation failures
 - Prompt user: "Implementation failed after <max_attempts> attempts. Continue to verification anyway? (yes/no)"
 - If user response == "no":
...
9. **Initialize Iteration Counter**
 ...
 - Set: max_iterations = max_verification_iterations
 - Determine: refactored_spec_path from implementer output

10. **Per Iteration** (while verification_iteration < max_iterations):
 ...
 h. **Check Loop Condition**
 - If verification_iteration >= max_iterations:
 - Break loop, proceed to step 11

11. **Verification Failure Escalation** (if max_iterations reached)
 ...
 - Prompt user: "Verification found <issue_count> remaining issues after <max_iterations> iterations. How to proceed? (continue/abort/retry)"
 - Handle response:
...
5. Max 3 attempts before user escalation
...
- **Condition**: implementation_attempt >= max_attempts OR verification_iteration >= max_iterations
- **Action**: Summarize all attempts and failures
...
3. **Use Agent Commands**: Each agent uses specific commands:
 - Analyzer: `/spec:analyze-spec`, `/refactor:plan-refactor`
 - Implementer: `/refactor:prepare-refactor`, `/refactor:apply-refactor`, `/refactor:validate-refactor`
 - Verifier: `/verify:detect-framework`, `/verify:analyze-behavior`, `/verify:design-tests`, `/verify:execute-verification`, `/verify:classify-results`
...
**v4.0.0 Changes**:
- Removed narrative sections and LLM jargon
- Added explicit "Objective" statement
- Changed to Input/Output Contract format
- Execution Procedure with 4 phases and 18 numbered steps
- Eliminated vague language ("coordinate agents" → specific agent invocation with exact task descriptions)
- Added Boolean decision criteria for all conditionals
- Error handling with Condition/Action/Recovery format
- Made feedback loop patterns explicit with triggering conditions
- Specified max attempt counts inline (3 for implementation, 2 for verification)
- Clarified cross-agent reflection process with query templates
- Added pipeline state tracking schema
- Included complete report template
- Specified exact agent task descriptions for invocations
