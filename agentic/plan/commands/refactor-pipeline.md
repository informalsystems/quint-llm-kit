...
description: Orchestrate complete refactor workflow with analyzer, implementer, verifier-light, and feedback loops
version: 4.0.0
---
...
Execute complete Quint specification refactoring workflow coordinating analyzer, implementer, and verifier-light agents with feedback loops, user approval, and cross-agent reflection for quality assurance.
...
10. **Per Iteration** (iterate until verification passes or cannot proceed):
 ...
 a. **Invoke Verifier-light Agent**
 - Increment: verification_iteration
 - Execute: Launch @verifier-light with task:
 ```
 ...
 - Execute: Launch @implementer with task:
 ```
 @implementer: Verifier-light found a bug in the refactored spec:
 - Issue: <issue.title>
 - Evidence: <issue.evidence>
 ...
 - Log: Feedback loop (verifier-light → implementer, reason: "spec_bug_fix")
 ...
 - Execute: Launch @verifier-light with task:
 ```
 @verifier-light: Some tests indicate potential gaps rather than spec bugs.
 Issues: <list of suspect/coverage_gap issues>

 Please improve test coverage for these scenarios and re-run verification.
 ```
 - Wait: Verifier-light completion
 - Log: Feedback loop (self → verifier-light, reason: "test_improvement")
 - Continue to next iteration (return to step 10a)
...
13. **Analyze Cross-Agent Alignment**
 ...
 - Compare: Analyzer expectations vs Verifier-light findings
...
14. **Count Feedback Loops**
 ...
 - Count: By type (analyzer→implementer, verifier-light→implementer, verifier-light→verifier-light)
...
🔄 Cross-Agent Reflection:
 ...
 - Analyzer ↔ Verifier-light: <aligned | issues_found>
...
### Pattern 2: Verifier-light Bug Fix Loop

**Trigger**: Tests fail due to spec logic errors (critical issues)

**Actors**: @verifier-light, @implementer, @analyzer

**Process**:
1. @verifier-light identifies spec bug with evidence
...
4. Re-run @verifier-light after fix
5. Max 2 iterations before user escalation

**Example**:
```
@verifier-light found: Invariant 'agreement' violated (quorum uses 3*f+1)
...
**Actors**: @verifier-light

**Process**:
1. Classify failing tests as coverage gaps
2. Request @verifier-light improve test coverage for specific scenarios
...
@verifier-light: "Increase --max-steps to 500 and add intermediate checks"
...
```json
{
 ...
 {
 "actors": ["analyzer", "verifier-light"],
...
- `Task`: Launch specialized agents (analyzer, implementer, verifier-light)
...
1. **Agent Usage Required**: Analyzer, implementer, and verifier-light agents MUST be used as described. Do not skip agents or perform their work directly.
...
- [ ] Verifier-light executed on all module instances
